...
# Placeholders so module-scope env checks pass while the bundler evaluates
# the app. Build stage only; the real values come from the pod's Secrets,
# which outrank an image ENV. Loopback on purpose — unusable at runtime.
...
USER 1000
EXPOSE 3000
CMD ["sh", "-c", "export PATH=/app/node_modules/.bin:$PATH; exec cross-env NODE_ENV=production node app.js"]
